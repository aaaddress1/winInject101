...
HANDLE get_process_handle(wchar_t proc_name[])
{
	...
	return OpenProcess(PROCESS_ALL_ACCESS, TRUE, process.th32ProcessID);
}
...
int main()
{
	...
	HANDLE access_token = get_process_handle(proc_name);
	...
	CreateRemoteThread(access_token, NULL, 0, (LPTHREAD_START_ROUTINE)mem, 0, 0, NULL);
	puts("finish!");
	getchar();
    return 0;
}
